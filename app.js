...
let transactions = JSON.parse(localStorage.getItem('ssilog_tx') || '[]');
...
function doLogin() {
  ...
  const users = JSON.parse(localStorage.getItem('ssilog_users') || '[]');
...
function showView(v) {
  ['dashboard','transactions','analysis','data'].forEach(id => {
    $('view'+id.charAt(0).toUpperCase()+id.slice(1)).style.display = id===v ? 'block' : 'none';
    $('nav'+id.charAt(0).toUpperCase()+id.slice(1))?.classList.toggle('active', id===v);
  });
...
function saveApiKey() { localStorage.setItem('ssilog_apikey', $('apiKeyInput').value.trim()); toast('✅ Đã lưu API Key!'); }
...
if (localStorage.getItem('ssilog_theme')==='dark') { document.body.classList.add('dark'); $('themeBtn').textContent='☀️'; }
...
function refreshDashboard() {
  ...
  $('metTotalCost').textContent = fmtVND(totalCost);
  $('metCurrentVal').textContent = fmtVND(totalCur);
  const pnlEl = $('metPnL'); pnlEl.textContent = fmtVND(pnl);
  pnlEl.className = 'metric-val '+(pnl>=0?'pos':'neg');
  const pctEl = $('metPnLPct'); pctEl.textContent = fmtPct(pnlPct);
  pctEl.className = 'metric-val '+(pnl>=0?'pos':'neg');
  $('metWins').textContent = wins.length+'/'+closed.length;
  $('metWinRate').textContent = winRate.toFixed(1)+'%';
  ...
  if (!stocks.length) {
    ...
    hList.innerHTML='<p class="empty-hint">Chưa có cổ phiếu</p>';
  } else {
    hBody.innerHTML = stocks.map(s=>{
      ...
      return `<tr><td><strong>${s}</strong></td><td>${h.qty.toLocaleString()}</td><td>${fmtVND(h.cost)}</td><td>${fmtVND(h.curPrice)}</td><td>${fmtVND(val)}</td><td class="${p>=0?'':''}"><span style="color:${p>=0?'var(--green)':'var(--red)'}">${fmtVND(p)}</span></td><td style="color:${pct>=0?'var(--green)':'var(--red)'}">${fmtPct(pct)}</td></tr>`;
    }).join('');
...
function drawCharts(holdings, txs) {
  ...
  if (sorted.length) {
    $('pnlEmpty').style.display='none'; pnlCtx.style.display='block';
    let cum=0;
    const labels=[], data=[];
    sorted.forEach(t=>{ cum+=(t.type==='SELL'?1:-1)*t.price*t.qty - (t.fee||0); labels.push(t.date); data.push(+cum.toFixed(0)); });
    pnlChartInst = new Chart(pnlCtx, { type:'line', data:{ labels, datasets:[{ label:'P&L tích lũy', data, borderColor:'#1a56db', backgroundColor:'rgba(26,86,219,.1)', fill:true, tension:.3, pointRadius:3 }] }, options:{ responsive:true, plugins:{legend:{display:false}}, scales:{ y:{ ticks:{ callback: v=>new Intl.NumberFormat('vi-VN',{notation:'compact'}).format(v)+'₫' } } } } });
  } else { $('pnlEmpty').style.display='block'; pnlCtx.style.display='none'; }
  ...
  const stocks = Object.keys(holdings);
  if (stocks.length) {
    $('allocEmpty').style.display='none'; allocCtx.style.display='block';
    const vals = stocks.map(s=>holdings[s].curPrice*holdings[s].qty);
    const colors=['#1a56db','#cc0000','#16a34a','#d97706','#7c3aed','#0891b2','#db2777'];
    allocChartInst = new Chart(allocCtx, { type:'doughnut', data:{ labels:stocks, datasets:[{ data:vals, backgroundColor:colors.slice(0,stocks.length), borderWidth:2 }] }, options:{ responsive:true, plugins:{ legend:{ position:'bottom', labels:{ font:{size:11} } } } } });
  } else { $('allocEmpty').style.display='block'; allocCtx.style.display='none'; }
}

// ── TRANSACTIONS ──────────────────────────────────────────────
function openAddTx() {
  editingTxId = null;
  $('txModalTitle').textContent = 'Thêm giao dịch mới';
...
function renderTxTable() {
  const filterStock = $('filterStock').value.trim().toUpperCase();
  ...
  const body = $('txBody');
  if (!list.length) { body.innerHTML='<tr><td colspan="9" class="empty-row">Không có giao dịch nào.</td></tr>'; return; }
...
async function runQuickAnalysis(type) {
  ...
  const panel = $('aiResult'); const content = $('aiResultContent');
  panel.style.display='block'; content.innerHTML='<div class="typing-dots"><div class="dot"></div><div class="dot"></div><div class="dot"></div></div>';
  const result = await callGemini(buildContext()+'\n\nYÊU CẦU: '+prompts[type]);
  content.innerHTML = parseMd(result);
}
...
async function callGemini(prompt) {
  ...
  try {
    ...
    if (!res.ok) { const e=await res.json(); return '❌ Lỗi API: '+(e.error?.message||res.status); }
...
function appendTyping() {
  const id='typ'+(++typCnt); const msgs=$('chatMsgs');
  const d=document.createElement('div'); d.className='chat-msg model'; d.id=id;
...
function importJSON(e){
  ...
  const r=new FileReader(); r.onload=ev=>{
    try{ const data=JSON.parse(ev.target.result); transactions=[...transactions,...data]; saveTxs(); toast('✅ Import thành công '+data.length+' GD!'); }
...
function importCSV(e){
  ...
  const r=new FileReader(); r.onload=ev=>{
    ...
    saveTxs(); renderTxTable(); toast('✅ Import '+cnt+' giao dịch!');
  }; r.readAsText(file,'UTF-8'); e.target.value='';
}
function clearAllData(){
  if(!confirm('⚠️ Xóa TOÀN BỘ dữ liệu? Hành động không thể hoàn tác!'))return;
  transactions=[]; saveTxs(); toast('🗑️ Đã xóa tất cả!'); renderTxTable();
}

// ── NAV IDs fix ───────────────────────────────────────────────
$('navDash').id='navDashboard'; // already correct but ensure
// Fix: use correct IDs
...
(function init(){
  ...
  if(sess){ try{ loginSuccess(JSON.parse(sess)); }catch{} }
  else { $('txDate').value=new Date().toISOString().slice(0,10); }
})();
